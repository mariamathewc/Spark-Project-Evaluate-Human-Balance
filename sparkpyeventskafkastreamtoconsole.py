...
stediStreamingDF = stediStreamingRawDF.selectExpr("cast(key as string) key", "cast(value as string) value")
...
stediStreamingDF \
.withColumn("value", from_json("value", stediEventSchema)).select(col("value.*")).createOrReplaceTempView('CustomerRisk')
